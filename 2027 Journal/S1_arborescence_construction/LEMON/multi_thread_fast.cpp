...
int calculate_arborescences_for_root(int root_id, string lgf_file_name){
...
int main(int argc, char** argv){
	...
	int max_thread_number = thread::hardware_concurrency();
    for (DNodeIt nit(g); nit != INVALID; ++nit){
		int root_id = g.id(nit);
		investigated_roots++;

	
		if(concurrent_threads < max_thread_number){
			thread t(calculate_arborescences_for_root,root_id,lgf_file_name);
			threads_per_root.push_back(std::move(t));
			concurrent_threads++;
		}
		else{
			for (auto& t : threads_per_root) {
				t.join();
			}
			threads_per_root.clear();
