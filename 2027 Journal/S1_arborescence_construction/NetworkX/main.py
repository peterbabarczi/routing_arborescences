...
for input_graph in os.listdir('../../S0_input_topologies'):
    Opt1 = True
    Opt2 = True
    Opt3 = True
    Opt4 = True
    ...
    if input_graph.endswith(".gml"): 
        xml_file = open('../../S2_routing_arborescences/' + str(input_graph[0:-4]) + "_O" + ("","1")[Opt1] + ("","2")[Opt2] + ("","3")[Opt3] + ("","4")[Opt4] + '.xml', 'w')
        G = fgu.construct_digraph_gml('../../S0_input_topologies/' + str(input_graph))
    elif(input_graph.endswith(".json")): 
        xml_file = open('../../S2_routing_arborescences/' + str(input_graph[0:-5]) + "_O" + ("","1")[Opt1] + ("","2")[Opt2] + ("","3")[Opt3] + ("","4")[Opt4] + '.xml', 'w')
        G = fgu.construct_digraph_json('../../S0_input_topologies/' + str(input_graph))
    else:
        continue
    ...
    fgu.save_graph_to_lgf(G,input_graph)

    start_time = time.process_time()

    investigated_roots = 0
    for root_node in Gi.nodes(): # [0]: 
