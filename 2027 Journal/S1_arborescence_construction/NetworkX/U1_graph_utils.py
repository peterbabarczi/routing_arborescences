...
def construct_digraph_gml_string_id(topology_name):
    G = nx.DiGraph()
    ...
    for node in GML.nodes(data=True):
        node_id[node[0]] =  node_name_to_id
        node_name_to_id += 1
        ...
        if 'Latitude' in node_data and 'Longitude' in node_data:
            ...
            coordinates[node_id[node[0]]] = (longitude,latitude)
        else:
            coordinates[node_id[node[0]]] = (int(10*rnd.random()), int(10*rnd.random()))
    ...
    for edge in GML.edges(data=True):
        G.add_edge(node_id[edge[0]],node_id[edge[1]])
        G.add_edge(node_id[edge[1]],node_id[edge[0]])
        edge_data = edge[2]
        edge_weight[node_id[edge[0]],node_id[edge[1]]] = 1 #round((float(edge_data['length'])),3)
        edge_weight[node_id[edge[1]],node_id[edge[0]]] = 1 #round((float(edge_data['length'])),3)
        # We store color as list
        edge_color[node_id[edge[0]],node_id[edge[1]]] = [0] 
        edge_color[node_id[edge[1]],node_id[edge[0]]] = [0] 
        # Capacity is used to store multiple instances of multi-edges
        edge_capacity[node_id[edge[0]],node_id[edge[1]]] = 1 
        edge_capacity[node_id[edge[1]],node_id[edge[0]]] = 1
...
def construct_digraph_gml(topology_name):
    ...
    for node in GML.nodes(data=True):
        ...
        if 'Latitude' in node_data and 'Longitude' in node_data:
            ...
            coordinates[int(node[0])] = (int(10*rnd.random()), int(10*rnd.random()))
...
def save_trees_to_json(Gi,root_node,input_graph,Opt1,Opt2,Opt3,Opt4):
    json_file = open('../../S2_routing_arborescences/json/' + str(input_graph[0:-4])  + "_O" + ("","1")[Opt1] + ("","2")[Opt2] + ("","3")[Opt3] + ("","4")[Opt4] + "_r" + str(root_node) + '.json', 'w')
    json_file.write(json.dumps(nx.node_link_data(Gi, edges="links")))
    json_file.close()

    json_graph = json.load(open('../../S2_routing_arborescences/json/' + str(input_graph[0:-4])  + "_O" + ("","1")[Opt1] + ("","2")[Opt2] + ("","3")[Opt3] + ("","4")[Opt4] + "_r" + str(root_node) + '.json', 'r'))
    Hi = nx.node_link_graph(json_graph, edges="links")

    return Hi

def save_graph_to_lgf(Gi,input_graph):
    lgf_file = open('../../S2_routing_arborescences/json/' + str(input_graph[0:-4]) + '.lgf', 'w')
    lgf_file.write("@nodes\nlabel\n")
    for n in Gi.nodes():
        lgf_file.write(str(n) + " \n")
    lgf_file.write("@edges\n\t\tlabel\n")
    edge_label = 0
    for u,v in Gi.edges():
        lgf_file.write(str(u) + "\t" + str(v) + "\t" + str(edge_label) +" \n")
        edge_label += 1
    lgf_file.close()  
