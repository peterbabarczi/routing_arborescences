...
def get_split_off_score(l):
    # Base score (negative/lower is better): 0, avoiding loop-edge: +1, creating bidirectional edge: -100 (-50 below because we count them twice)
    score = 0
    for v1,w1 in l:
        if v1 == w1: score += 1
        else: 
...
def split_off_node(Gi,root_node,so,optimize):
    ...
    unique_pairings = []
    for l in pairings:
        if l not in unique_pairings:
            unique_pairings.append(l)

    # Calculate score first for all permunations in pairings.
    scores = []
    for l in unique_pairings: 
        score = (1,get_split_off_score(l))[optimize]
        scores.append(score)

    ordered_pairings = sorted(zip(scores,unique_pairings), key=lambda x: x[0])
    # print(ordered_pairings)

   # Find the best possible pairing to split-off a given node
    for s,l in ordered_pairings:
        Gtest = Gi.copy()
        ...
        if(valid_split_off):
            ...
            # print("\t\tValid split-off: " + str(Gtest.nodes()[so]['split_pairs']) + " with score " + str(s))
            return Gtest
